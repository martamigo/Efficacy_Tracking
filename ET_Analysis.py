...
# -*- coding: utf-8 -*-
"""
Reconstructed and cleaned efficacy-tracking analysis script.

This file was rebuilt from the uploaded ET_Analysis.py, the manuscript results in
Migo_ET_v6.docx, and the fixed code fragments developed in prior conversations.

Main goals:
1. Keep only analyses needed to reproduce the manuscript's reported results/figures.
2. Remove duplicated notebook/debug/exploratory cells.
3. Use the corrected covariate structure:
   - Group ANCOVAs: age, sex, and condition order where relevant.
   - Continuous symptom regressions: DARS and RRS entered together, plus age, sex,
     and condition order where relevant; no redundant Group covariate.
   - FinalQ/item models: control for DARS, RRS, age, sex, and condition order where relevant.
4. Keep the fixed figure titles/labels and FinalQ item wording.

Run from terminal:
    python ET_Analysis_reconstructed_clean.py

You can edit the paths in CONFIG below.
"""
...
DATA_DIR = Path("/Users/martamigocortes/Desktop/Academics/Projectes/Efficacy_Tracking/data")
...
OUTPUT_DIR = Path("/Users/martamigocortes/Desktop/Academics/Projectes/Efficacy_Tracking/analysis_outputs")
...
def make_final_df(
    ...
    """
    q = collapse_one_row(questionnaires)
...
def figure1_task_performance_by_group(final_df: pd.DataFrame, save=True):
    dvs = ["sse_total", "sse_low", "sse_high"]
    fig, axes = plt.subplots(1, 3, figsize=(8.6, 2.9), constrained_layout=True)
    stats_rows = []

    for ax, dv in zip(axes, dvs):
        dat = final_df[final_df["Group_plot"].isin(GROUP_ORDER)].dropna(subset=[dv, "Group_plot"])
        means = dat.groupby("Group_plot", observed=True)[dv].mean().reindex(GROUP_ORDER)
        sems = dat.groupby("Group_plot", observed=True)[dv].sem().reindex(GROUP_ORDER)
        x = np.arange(len(GROUP_ORDER))
        ax.bar(x, means.values, yerr=sems.values, capsize=3, width=0.65)
        ax.set_xticks(x)
        ax.set_xticklabels(GROUP_ORDER)
        ax.set_title(TTITLES[dv], fontweight="bold")
        ax.set_ylabel(Y_LABELS[dv])
        beautify_axes(ax)

        model, aov = run_group_ancova(final_df, dv, include_order=True)
        Fv = aov.loc["C(Group_plot)", "F"]
        p = aov.loc["C(Group_plot)", "PR(>F)"]
        df1 = aov.loc["C(Group_plot)", "df"]
        df2 = model.df_resid
        ax.text(0.03, 0.98, f"F({int(df1)}, {int(df2)}) = {Fv:.2f}\np = {p:.3g}", transform=ax.transAxes, va="top")
        stats_rows.append({"DV": dv, "F": Fv, "df1": df1, "df2": df2, "p": p})

    fig.suptitle("Task performance by group", fontweight="bold")
...
def figure2_condition_difference_regressions(final_df: pd.DataFrame, save=True):
    ...
    # Corrected: no Group covariate; DARS and RRS in the same model.
    formula = f"{dv} ~ DARS_Score + RRS_Score + Age + C(Sex) + avg_condition_order"
    model = smf.ols(formula, data=dat).fit()
    print("\n=== Figure 2 model: condition performance difference ===")
    print(model.summary())

    fig, axes = plt.subplots(1, 2, figsize=(7.0, 3.0), constrained_layout=True)
    specs = [("DARS_Score", "DARS score (lower = more severe anhedonia)", "A  Anhedonia and condition-difference error"),
             ("RRS_Score", "RRS score (higher = more severe rumination)", "B  Rumination and condition-difference error")]

    for ax, (xcol, xlabel, title) in zip(axes, specs):
        covars_y = [c for c in ["Age", "C(Sex)", "avg_condition_order", "RRS_Score" if xcol == "DARS_Score" else "DARS_Score"]]
        covars_x = [c for c in ["Age", "C(Sex)", "avg_condition_order", "RRS_Score" if xcol == "DARS_Score" else "DARS_Score"]]
        y_res = smf.ols(f"{dv} ~ " + " + ".join(covars_y), data=dat).fit().resid
        x_res = smf.ols(f"{xcol} ~ " + " + ".join(covars_x), data=dat).fit().resid
        ax.scatter(x_res, y_res, s=22, alpha=0.45)
        lr = stats.linregress(x_res, y_res)
        xs = np.linspace(np.nanmin(x_res), np.nanmax(x_res), 100)
        ax.plot(xs, lr.intercept + lr.slope * xs, lw=2)
        ax.axhline(0, linestyle="--", linewidth=0.8)
        ax.axvline(0, linestyle="--", linewidth=0.8)
        ax.set_title(title, loc="left", fontweight="bold")
        ax.set_xlabel(f"{xlabel}\n(residualized)")
        ax.set_ylabel("High − low efficacy-tracking error\n(residualized)")
        focal = model.params.get(xcol, np.nan)
        p = model.pvalues.get(xcol, np.nan)
        ax.text(0.03, 0.97, f"b = {focal:.3f}\np = {p:.3g}", transform=ax.transAxes, va="top")
        beautify_axes(ax)
...
def figure3_model_fit(fit_df: pd.DataFrame, final_df: pd.DataFrame, save=True):
    ...
    ax1 = axes[0]
    beautify_axes(ax1)
    ax1.plot([0, 1], [0, 1], linestyle="--", linewidth=1.2, alpha=0.8)
    centers, y_mean, y_se = binned_means(fit_plot["predicted"], fit_plot["observed"], bins=20)
    m = np.isfinite(y_mean)
    ax1.errorbar(centers[m], y_mean[m], yerr=y_se[m], fmt="o-", markersize=5, linewidth=2.2, capsize=3)
    ax1.set_xlim(0, 1)
    ax1.set_ylim(0, 1)
    ...
    ax2 = axes[1]
    beautify_axes(ax2)
    tmp = rmse_plot[rmse_plot["Group_plot"].isin(GROUP_ORDER)].copy()
    stats_df = tmp.groupby("Group_plot", observed=True)["RMSE"].agg(["mean", "std", "count"]).reindex(GROUP_ORDER)
    stats_df["se"] = stats_df["std"] / np.sqrt(stats_df["count"])
    x = np.arange(len(GROUP_ORDER))
    ax2.bar(x, stats_df["mean"], width=0.7)
    ax2.errorbar(x, stats_df["mean"], yerr=stats_df["se"], fmt="none", capsize=3)
    ax2.set_xticks(x)
...
def partial_plot(ax, data, dv, focal, control_label, title):
    ...
    ax.scatter(x_res, y_res, s=22, alpha=0.42)
    lr = stats.linregress(x_res, y_res)
    xs = np.linspace(np.nanmin(x_res), np.nanmax(x_res), 100)
    ax.plot(xs, lr.intercept + lr.slope * xs, lw=2)
    ax.axhline(0, linestyle="--", linewidth=0.8)
    ax.axvline(0, linestyle="--", linewidth=0.8)
    ax.set_title(title, loc="left", fontweight="bold")
    ax.set_xlabel(f"{focal.replace('_Score','')} residualized\n(controlling {control_label})")
    ax.set_ylabel(f"{PRETTY_OUTCOME.get(dv, dv)} residualized")
    beautify_axes(ax)


def figure4_learning_rate_associations(final_df: pd.DataFrame, save=True):
    df = final_df.copy()
    # Learning rates are bounded/skewed, so keep the corrected transformed/sensitivity-oriented approach.
    results = []
    ...
    print("\n=== Figure 4 learning-rate models ===")
    print(res.to_string(index=False))

    fig, axes = plt.subplots(3, 2, figsize=(7.2, 7.5), constrained_layout=True)
    plot_specs = [
        ("Alpha_p", "DARS_Score", "RRS", "A  DARS and positive learning rate"),
        ("Alpha_p", "RRS_Score", "DARS", "B  RRS and positive learning rate"),
        ("Alpha_n", "DARS_Score", "RRS", "C  DARS and negative learning rate"),
        ("Alpha_n", "RRS_Score", "DARS", "D  RRS and negative learning rate"),
        ("alpha_diff", "DARS_Score", "RRS", "E  DARS and learning-rate difference"),
        ("alpha_diff", "RRS_Score", "DARS", "F  RRS and learning-rate difference"),
    ]
    for ax, (dv, focal, control, title) in zip(axes.ravel(), plot_specs):
        dat = df.dropna(subset=[dv, focal, "Age", "Sex", "avg_condition_order", "RRS_Score" if focal == "DARS_Score" else "DARS_Score"])
        partial_plot(ax, dat, dv, focal, control, title)
    plt.show()
    return res
...
def run_finalq_factor_analysis(final_df: pd.DataFrame, save=True):
    ...
    obs_evals = np.sort(np.real(np.linalg.eig(corr_mat)[0]))[::-1]
    rng = np.random.default_rng(RANDOM_SEED)
    rand_evals = []
    for _ in range(N_PARALLEL):
        rand = rng.normal(size=X.shape)
        rand_corr = np.corrcoef(rand, rowvar=False)
        rand_evals.append(np.sort(np.real(np.linalg.eig(rand_corr)[0]))[::-1])
    rand_evals = np.vstack(rand_evals)
    rand_mean = rand_evals.mean(axis=0)
    rand_95 = np.percentile(rand_evals, 95, axis=0)
    ...
    # Figure 5: latent structure + associations.
    fig, axes = plt.subplots(1, 3, figsize=(12.5, 3.6), gridspec_kw={"width_ratios": [1, 1.05, 1.7]}, constrained_layout=True)

    ax = axes[0]
    ax.plot(range(1, len(obs_evals)+1), obs_evals, "o-", label="Observed")
    ax.plot(range(1, len(rand_95)+1), rand_95, "o--", label="Random 95th pct")
    ax.set_xlabel("Factor number")
    ax.set_ylabel("Eigenvalue")
    ax.set_title("A  Parallel analysis", loc="left", fontweight="bold")
    ...
    ax.barh(load_plot["Item"], load_plot["Loading"])
    ax.axvline(0, color="black", linewidth=1)
    ax.set_xlabel("Factor loading")
    ax.set_title("B  Loadings", loc="left", fontweight="bold")
    beautify_axes(ax)

    ax = axes[2]
    coef_df = results.copy()
    coef_df["plot_label"] = coef_df["Predictor"].map(PRETTY_OUTCOME).fillna(coef_df["Predictor"])
    coef_df = coef_df.sort_values("b")
    ypos = np.arange(len(coef_df))
    for i, (_, row) in enumerate(coef_df.iterrows()):
        color = PURPLE if row["sig_FDR"] else LIGHT_PURPLE
        ax.errorbar(row["b"], i, xerr=[[row["b"] - row["CI_low"]], [row["CI_high"] - row["b"]]], fmt="o", color=color, ecolor=color, capsize=3)
    ax.axvline(0, color="black", linewidth=1)
    ax.set_yticks(ypos)
    ax.set_yticklabels(coef_df["plot_label"])
    ax.set_xlabel("Regression coefficient")
    ax.set_title("C  Associations with task measures", loc="left", fontweight="bold")
    beautify_axes(ax)
    plt.show()
...
def main():
    ...
    compare_learning_rates_by_group(final_df)
    ...
    figure1_task_performance_by_group(final_df)
    figure2_condition_difference_regressions(final_df)
    figure3_model_fit(fit_df, final_df)
    figure4_learning_rate_associations(final_df)

    if HAS_FACTOR_ANALYZER:
        run_finalq_factor_analysis(final_df)
    else:
        print("Skipping Figure 5 factor analysis because factor_analyzer is not installed.")

    supplemental_finalq_group_means(final_df)
    finalq_item_level_models(final_df)
