...
def figure_aligned_group_efficacy_trajectories(
    clean_data: pd.DataFrame,
    final_df: pd.DataFrame,
    save: bool = True,
) -> pd.DataFrame:
    """Plot group-average reported efficacy after aligning condition order.

    Participants completed the task in one of two orders: low-to-high efficacy or
    high-to-low efficacy. For this visualization, high-to-low participants are
    mirrored so every participant is plotted on a common low-to-high ground-truth
    axis. The plot shows group means with +/-1.96 SE bands and the aligned
    ground-truth trajectory.
    """
    group_info = final_df[["ID", "Group_plot", "avg_condition_order"]].copy()
    group_info["ID"] = group_info["ID"].astype(str)

    d = clean_data.merge(group_info, on="ID", how="inner", validate="many_to_one").copy()
    d = d.dropna(subset=["Eff_Rating", "True_Perc", "Group_plot", "avg_condition_order"]).copy()
    d = d.sort_values(["ID", "trial"])
    d["rating_trial"] = d.groupby("ID").cumcount() + 1
    d["aligned_trial"] = np.where(
        pd.to_numeric(d["avg_condition_order"], errors="coerce").eq(1),
        33 - d["rating_trial"],
        d["rating_trial"],
    )
    d["mirrored_for_plot"] = pd.to_numeric(d["avg_condition_order"], errors="coerce").eq(1)
    d["Group_plot"] = pd.Categorical(d["Group_plot"].astype(str), categories=GROUP_ORDER, ordered=True)

    summary = (
        d.groupby(["Group_plot", "aligned_trial"], observed=True)
        .agg(
            mean_eff=("Eff_Rating", "mean"),
            sd_eff=("Eff_Rating", "std"),
            n=("ID", "nunique"),
        )
        .reset_index()
    )
    summary["se_eff"] = summary["sd_eff"] / np.sqrt(summary["n"])
    summary["ci_low"] = (summary["mean_eff"] - 1.96 * summary["se_eff"]).clip(lower=0)
    summary["ci_high"] = (summary["mean_eff"] + 1.96 * summary["se_eff"]).clip(upper=1)

    truth = (
        d.groupby("aligned_trial", observed=True)
        .agg(True_Perc=("True_Perc", "mean"))
        .reset_index()
        .sort_values("aligned_trial")
    )
    counts = d[["ID", "Group_plot"]].drop_duplicates().groupby("Group_plot", observed=True).size()
    mirror_counts = (
        d[["ID", "Group_plot", "mirrored_for_plot"]]
        .drop_duplicates()
        .groupby(["Group_plot", "mirrored_for_plot"], observed=True)
        .size()
    ...
    for group in GROUP_ORDER:
        s = summary[summary["Group_plot"].astype(str).eq(group)].sort_values("aligned_trial")
        if s.empty:
            continue
        color = GROUP_COLORS.get(group, "tab:gray")
        ax.fill_between(
            s["aligned_trial"].to_numpy(dtype=float),
            s["ci_low"].to_numpy(dtype=float),
            s["ci_high"].to_numpy(dtype=float),
            color=color,
            alpha=0.18,
            linewidth=0,
        )
    ...
    ax.plot(
        truth["aligned_trial"].to_numpy(dtype=float),
        truth["True_Perc"].to_numpy(dtype=float),
        color="black",
        linewidth=2.8,
        linestyle="--",
        label="Ground truth",
    ...
    for group in GROUP_ORDER:
        ...
        ax.plot(
            s["aligned_trial"].to_numpy(dtype=float),
            s["mean_eff"].to_numpy(dtype=float),
            color=color,
            linewidth=3.2,
            label=f"{group} group average (n={int(counts.loc[group])})",
        )

    note = (
        "Note: participants who saw High -> Low first were mirrored/reversed for this plot\n"
    ...
    if save:
        fig.savefig(OUTPUT_DIR / "effect_trajectories_combined_aligned_group_bands.png", dpi=600, bbox_inches="tight")
        summary.to_csv(OUTPUT_DIR / "effect_trajectories_combined_aligned_group_mean_bands.csv", index=False)
        truth.to_csv(OUTPUT_DIR / "effect_trajectories_combined_aligned_ground_truth.csv", index=False)
        mirror_counts.to_csv(OUTPUT_DIR / "effect_trajectories_combined_aligned_mirroring_counts.csv", index=False)
    plt.close(fig)
    return summary
...
def main():
    ...
    figure_aligned_group_efficacy_trajectories(clean_data, final_df)
